...
import datetime
import hashlib
from flask import Flask, render_template, jsonify, request, redirect, url_for, abort #https://m.blog.naver.com/dsz08082/222025157731 - 특정 ip 차단from werkzeug.utils import secure_filename
...
def ADMIN_withdraw_list():
   ...
   if payload['admin'] == 'yes':
       ...
       return render_template('ADMIN_withdraw_list.html', mynickname=mynickname, login_time=login_time,withdraw_all=withdraw_all)
   else:
...
def recordpaper(number):
   ...
   print(myFeed)
   me_mentor = db.mentor.find_one({"number": payload["number"]})
   me_menti = db.menti.find_one({"number": payload["number"]})
   ...
   like_info = db.like.find_one({"number": number, "category": "recordpaper"})
   if like_info:
   ...
   like_check = db.like.find_one({"number": number, "category": "recordpaper", "who": payload["number"]})
   ...
   bookmark_check = db.bookmark.find_one(
       {"number": number, "category": "recordpaper", "who": payload["number"]})
   if bookmark_check is not None:
       bookmark_check = 'mark'
   else:
       ...
       if not myFeed:
           ...
           if time is None:
               ...
               if date_diff.seconds > 3600:
                   ...
                   db.recordpaper.update_one({"number": int(number)}, {"$inc": {'visit': 1}})
       return render_template('recordpaper.html', bookmark_check=bookmark_check, like_check=like_check, like_count=like_count, status=status, me_info=me_info, mentor_info=mentor, data_num=data_number, record=recordpaper_info, mentorinfo=mentorinfo, follower=mentor_follower, followed=followed, token_receive=token_receive, action_mentor=action_mentor_array, nonaction_mentor=nonaction_mentor_array, myFeed=myFeed)
   except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
       myFeed = False
       return render_template('recordpaper.html', mentor_info=mentor, record=recordpaper_info, mentorinfo=mentorinfo, myFeed=myFeed, follower=mentor_follower)
...
def resume(number, time):
   ...
   like_info = db.like.find_one({"number": number, "category": "resume", "time": time})
   like_count = len(like_info["who"])
   print(like_count)

   like_check = db.like.find_one({"number": number, "category": "resume", "time": time, "who": payload["number"]})
   if like_check is not None:
       like_check = 'like'
   else:
       like_check = 'unlike'
   ...
   bookmark_check = db.bookmark.find_one(
       {"number": number, "category": "resume", "time": time, "who": payload["number"]})
   if bookmark_check is not None:
       bookmark_check = 'mark'
   else:
       bookmark_check = 'unmark'
       ...
       if not myFeed:
           ...
           if time1 is None:
               ...
               if date_diff.seconds > 10:
                   ...
                   db.resume.update_one({"number": int(number), "time": time}, {"$inc": {'visit': 1}})
       return render_template('resume.html', bookmark_check=bookmark_check, like_check=like_check ,like_count=like_count, status=status, me_info=me_info, mentor_info=mentor, data_num=data_number, resume=resume_info, mentorinfo=mentorinfo, follower=mentor_follower, followed=followed, token_receive=token_receive, action_mentor=action_mentor_array, nonaction_mentor=nonaction_mentor_array, myFeed=myFeed)
   except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
       myFeed = False
       return render_template('resume.html', mentor_info=mentor, resume=resume_info, mentorinfo=mentorinfo, myFeed=myFeed, follower=mentor_follower)
...
def story(number, time):
   ...
   like_info = db.like.find_one({"number": number, "category": "story", "time": time})
   like_count = len(like_info["who"])
   print(like_count)

   like_check = db.like.find_one({"number": number, "category": "story", "time": time, "who": payload["number"]})
   if like_check is not None:
       like_check = 'like'
   else:
       like_check = 'unlike'
   ...
   bookmark_check = db.bookmark.find_one({"number": number, "category": "story", "time": time, "who": payload["number"]})
   if bookmark_check is not None:
       bookmark_check = 'mark'
   else:
       bookmark_check = 'unmark'
       ...
       if not myFeed:
           ...
           if time1 is None:
               ...
               if date_diff.seconds > 10:
                   ...
                   db.story.update_one({"number": int(number), "time": time}, {"$inc": {'visit': 1}})
       return render_template('story.html', bookmark_check=bookmark_check, like_check=like_check, like_count=like_count, status=status, me_info=me_info, mentor_info=mentor, data_num=data_number, story=story_info, mentorinfo=mentorinfo, follower=mentor_follower, followed=followed, token_receive=token_receive, action_mentor=action_mentor_array, nonaction_mentor=nonaction_mentor_array, myFeed=myFeed)
   except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
       myFeed = False
       return render_template('story.html', mentor_info=mentor, story=story_info, mentorinfo=mentorinfo, myFeed=myFeed, follower=mentor_follower)
...
def recordpaper_post(number):
   ...
   return render_template('recordpaper_post.html',me_info=me_info, mentor_info=mentor_info, recordpaper_info=recordpaper_info, record_info=record_info,action_mentor=action_mentor_array, nonaction_mentor=nonaction_mentor_array, status=status,
...
def recordpaper_post_chart(number):
   ...
   if int(number) == int(payload["number"]):
       ...
       return jsonify({'result': 'success', 'chartJsArray': recordpaper_find['chart_js_array'],
                       'record_info': recordpaper_find['record_info']})
...
def resume_post(number, time):
    ...
    me_info = mentor_info
    status = 'mentor'
    resume_info = db.resume.find_one({"number": payload["number"]})
    ...
    return render_template('resume_post.html',time=time, me_info=me_info, mentor_info=mentor_info,mentorinfo_info=mentorinfo_info,resume_info=resume_info,action_mentor=action_mentor_array, nonaction_mentor=nonaction_mentor_array, status=status,
                           my_alert=my_alert, token_receive=token_receive)
...
def story_post(number, time):
   ...
   if number == payload['number']:
       ...
       return render_template('story_post.html',me_info=me_info,status=status, mentor_info=mentor_info, mentorinfo_info=mentorinfo_info, story_info=story_info,action_mentor=action_mentor_array, nonaction_mentor=nonaction_mentor_array, my_alert=my_alert, token_receive=token_receive)
   else:
       return redirect(url_for("index"))
...
def menti_mypage_mydata(nickname):
   ...
   for document in my_data_all:
       ...
       if document['category'] == 'recordpaper':
           ...
           if db.reply.find_one({'number':int(document['mentor_num']), 'category':'recordpaper'}) is not None:
               reply = str(db.reply.find_one({'number':int(document['mentor_num']), 'category':'recordpaper'})['reply']).count('일')
           else:
               reply = 0

           if db.visit.find_one({'to_number':int(document['mentor_num']), 'category':'recordpaper','from_number':int(payload["number"]) }) is not None:
               time = db.visit.find_one({'to_number':int(document['mentor_num']), 'category':'recordpaper','from_number':int(payload["number"]) })['current_time'][-1]
           else:
               time = 'no'
           ...
           category = 'resume'
           miniTab = document['miniTab']
           title = db.resume.find_one({'number':int(document['mentor_num']), 'time':document['time']})['resume_title']

           if db.like.find_one({'number': int(document['mentor_num']), 'category': 'resume', 'time':document['time']}) is not None:
               like = len(db.like.find_one({'number': int(document['mentor_num']), 'category': 'resume', 'time':document['time']})['who'])
           else:
               like = 0

           if db.reply.find_one({'number': int(document['mentor_num']), 'category': 'resume', 'time':document['time']}) is not None:
               reply = str(db.reply.find_one({'number': int(document['mentor_num']), 'category': 'resume', 'time':document['time']})['reply']).count('일')
           else:
               reply = 0

           if db.visit.find_one({'to_number': int(document['mentor_num']), 'category': 'resume', 'time':document['time'],'from_number': int(payload["number"])}) is not None:
               time = db.visit.find_one({'to_number': int(document['mentor_num']), 'category': 'resume', 'time':document['time'],'from_number': int(payload["number"])})['current_time'][-1]
           else:
               visit = 'no'

           img = db.mentor.find_one({'number': int(document['mentor_num'])})['profile_pic_real']
           univ = db.resume.find_one({'number': int(document['mentor_num']),'time':document['time']})['resume_univ']
           major = db.resume.find_one({'number': int(document['mentor_num']),'time':document['time']})['resume_major']
           student_num = db.resume.find_one({'number': int(document['mentor_num']),'time':document['time']})['resume_class']
           type = db.resume.find_one({'number': int(document['mentor_num']),'time':document['time']})['resume_type']
           price = db.resume.find_one({'number': int(document['mentor_num']),'time':document['time']})['resume_price']
           # exp = db.pay.find_one({ 'time':document['time'],'number':int(document['mentor_num']),'client_num':int(payload["number"])})['exp_time']
           exp = "2021년 11월 56일"

       doc={
           'category':category,'miniTab':miniTab,'title':title,'like':like,'reply':reply, 'time':time,'img':img, 'univ':univ,'major':major,'student_num':student_num,'type':type,'price':price,'exp':exp
       }
       # print(doc)
       my_data.append(doc)
   ...
   return render_template('menti_mypage_mydata.html',param=i,my_data=my_data,menti_info=menti_info, me_info=me_info,action_mentor=action_mentor_array,nonaction_mentor=nonaction_mentor_array,status=status,my_alert=my_alert,token_receive=token_receive)
...
def menti_mypage_mystory(nickname):
   ...
   for document in my_data_all:
       ...
       if db.like.find_one({'number': int(document['mentor_num']), 'category': 'story', 'time': document['time']}):
           like = len(db.like.find_one({'number': int(document['mentor_num']), 'category': 'story', 'time': document['time']})['who'])
       else:
           like = 0

       if db.reply.find_one({'number': int(document['mentor_num']), 'category': 'story', 'time': document['time']}):
           reply = str(db.reply.find_one({'number': int(document['mentor_num']), 'category': 'story', 'time': document['time']})['reply']).count('일')
       else:
           reply =0

       if db.visit.find_one({'to_number': int(document['mentor_num']), 'category': 'story', 'time': document['time'],'from_number': int(payload["number"])}) is not None:
           time = db.visit.find_one({'to_number': int(document['mentor_num']), 'category': 'story', 'time': document['time'],'from_number': int(payload["number"])})['current_time'][-1]
       else:
           time = 'no'
       ...
       doc={
           'category':category,'miniTab':miniTab,'title':title,'cat':cat,'like':like,'reply':reply, 'time':time,'img':img, 'univ':univ,'major':major,'student_num':student_num,'type':type,
       }
       # print(doc)
       my_data.append(doc)
   ...
   return render_template('menti_mypage_mystory.html',param=i,my_data=my_data,menti_info=menti_info, me_info=me_info,action_mentor=action_mentor_array,nonaction_mentor=nonaction_mentor_array,status=status,my_alert=my_alert,token_receive=token_receive)
...
def menti_mypage_info(nickname):
   ...
   if nickname == payload["nickname"]:
       ...
       return render_template('menti_mypage_info.html', menti_info=menti_info, me_info=menti_info, action_mentor=action_mentor_array,nonaction_mentor=nonaction_mentor_array,status=status,my_alert=my_alert,token_receive=token_receive)
...
def menti_withdraw(number):
   ...
   return render_template('menti_withdraw.html', menti_info=menti_info, me_info=menti_info, action_mentor=action_mentor_array,nonaction_mentor=nonaction_mentor_array,status=status,my_alert=my_alert,token_receive=token_receive)
...
def mentor_mypage_mydata(nickname):
    ...
    for resume in resume_list:
        if db.like.find_one({'number':mentor_number, 'category':'resume', 'time':resume['time']}):
            resume_like = len(db.like.find_one({'number':mentor_number, 'category':'resume', 'time':resume['time']})['who'])
        else:
            resume_like = 0
        if db.reply.find_one({'number':mentor_number, 'category':'resume', 'time':resume['time']}):
            resume_reply = str(db.reply.find_one({'number':mentor_number, 'category':'resume', 'time':resume['time']})['reply']).count('일')
        else:
            resume_reply = 0
        resume_array.append([resume, resume_like, resume_reply])

    return render_template('mentor_mypage_mydata.html',recordpaper=recordpaper,resume_array=resume_array, mentor_info=mentor_info,mentorinfo_info=mentorinfo_info, me_info=me_info,token_receive=token_receive, action_mentor=action_mentor_array,
    ...
    if nickname == payload['nickname']:
        ...
        for story in story_list:
            if db.like.find_one({'number': mentor_number, 'category': 'story', 'time': story['time']}):
                story_like = len(db.like.find_one({'number': mentor_number, 'category': 'story', 'time': story['time']})['who'])
            else:
                story_like = 0
            if db.reply.find_one({'number': mentor_number, 'category': 'story', 'time': story['time']}):
                story_reply = str(db.reply.find_one({'number': mentor_number, 'category': 'story', 'time': story['time']})['reply']).count('일')
            else:
                story_reply = 0
...
def user_mentor(nickname):
   ...
   try:
       ...
       product_number = db.recordpaper.count({"number": mentor_num, "release":"sell"}) + db.resume.count({"number": mentor_num, "release":"sell"})
       ...
       sorted_mentor_story = list(db.story.find({"number": mentor_num}).sort('time', -1))[:8]
       print(sorted_mentor_story)
       story_number = db.story.count({"number": mentor_num})
       mentor_story = []
       for story in sorted_mentor_story:
           ...
           #story_like = len(
           #    db.like.find_one({'number': mentor_num, 'category': 'story', 'time': story_time})['who'])
       ...
       return render_template('user_mentor.html', mentor_record=mentor_record, mentor_resume=mentor_resume, product_number=product_number, story_number=story_number, mentor_story=mentor_story, mentor_info=mentor_info, mentorinfo_info=mentorinfo_info, status=status, chart_array=user_mentor_chart, myFeed=myFeed, resume=mentor_resume, record=mentor_recordpaper, me_info=me_info, follower=mentor_follower, followed=followed, token_receive=token_receive, action_mentor=action_mentor_array, nonaction_mentor=nonaction_mentor_array, my_alert=my_alert, admin=admin)
   except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
       print('no token')
       myFeed = False
       ...
       for record in sorted_hot_record:
           if record['record_info'] != '':
               ...
               db_mentorinfo3 = db.mentor_info.find_one({'number': mentor_num3},
                                                        {'_id': False, 'mentor_univ': True, 'mentor_major': True,
                                                         'mentor_number': True, 'mentor_type': True})
               db_like = db.like.find_one({'number': mentor_num3, 'category': 'recordpaper'})

               record_title = record['record_title']
               record_price = record['record_price']
               if db_like is not None:
                   record_likely = len(db_like['who'])
               else:
                   record_likely = 0
...
def resume_save(number, time):
   ...
   try:
       ...
       resume_info_1_receive = request.form["resume_info_1_give"]
       resume_info_2_receive = request.form["resume_info_2_give"]
       resume_info_3_receive = request.form["resume_info_3_give"]
       resume_info_4_receive = request.form["resume_info_4_give"]
       ...
       if db.resume.find_one({'number':number, 'time':time}) is not None:
           doc={
               ...
               "resume_info": [resume_info_1_receive, resume_info_2_receive, resume_info_3_receive,
                               resume_info_4_receive],
           ...
           doc = {
               ...
               "resume_class": resume_number_receive,
               "resume_desc": resume_desc_receive,
               "resume_number": [resume_1_receive, resume_2_receive, resume_3_receive, resume_4_1_receive, resume_4_2_receive, resume_4_3_receive],
               "resume_info": [resume_info_1_receive,resume_info_2_receive,resume_info_3_receive,resume_info_4_receive],
               "resume_price": resume_price_receive,
...
def story_sellyes():
   ...
   try:
       ...
       db.story.update_one({'id': payload['id']}, {'$set': doc})
       return jsonify({"result": "success", 'msg': '프로필을 업데이트했습니다.'})
   except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
       return redirect(url_for("home"))


@app.route('/resume_sellyes', methods=['POST'])
def resume_sellyes():
   token_receive = request.cookies.get('mytoken')
   try:
       payload = jwt.decode(token_receive, SECRET_KEY, algorithms=['HS256'])
       ...
       db.resume.update_one({'id': payload['id']}, {'$set': doc})
       return jsonify({"result": "success", 'msg': '프로필을 업데이트했습니다.'})
   except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
       return redirect(url_for("home"))


@app.route('/recordpaper_sellyes/<int:number>/<time>', methods=['POST'])
def recordpaper_sellyes(number, time):
   token_receive = request.cookies.get('mytoken')
   try:
       payload = jwt.decode(token_receive, SECRET_KEY, algorithms=['HS256'])
       sell = request.form["sell"]
       ...
       db.resume.update_one({'number': number, 'time':time}, {'$set': doc})
       return jsonify({"result": "success"})
   except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
       return redirect(url_for("home"))
...
def recordpaper_get():
   ...
   find_record = db.recordpaper.find_one({'number': number_receive})
   find_mentor = db.mentor.find_one({'number': number_receive})
   find_info = db.mentor_info.find_one({'number': number_receive})

   mentor_rec = find_record['record_info']
   mentor_comment = find_record['record_comment']
   mentor_title = find_record['record_title']
   ...
   return jsonify({"result": "success", "record_reply": record_reply_array, "mentor_rec": mentor_rec, "mentor_comment": mentor_comment, "mentor_title": mentor_title, "mentor_nickname": mentor_nickname, "mentor_tag": mentor_tag, "mentor_univ": mentor_univ, "mentor_major": mentor_major, "mentor_number": mentor_number, "mentor_type": mentor_type})


@app.route('/resume_get', methods=['GET'])
def resume_get():
   number_receive = int(request.args.get('number_give'))
   time_receive = str(request.args.get('time_give'))
   print(time_receive)
   ...
   resume_reply = list(db.reply.find({'number': number_receive, 'category': 'resume'}))
   # print(community_reply)
   resume_reply_array = []
   for reply in resume_reply:
       resume_reply_array.append([reply['time'], reply['reply']])
   return jsonify({"result": "success", "resume_reply": resume_reply_array, "mentor_tag": mentor_tag})
...
def story_get():
   ...
   story_reply = list(db.reply.find({'number': number_receive, 'category': 'story'}))
   # print(community_reply)
   story_reply_array = []
   for reply in story_reply:
       story_reply_array.append([reply['time'], reply['reply']])
   return jsonify({"result": "success", "story_reply": story_reply_array, "mentor_story": mentor_story, "mentor_title": mentor_title, "mentor_nickname": mentor_nickname, "mentor_tag": mentor_tag, "mentor_univ": mentor_univ, "mentor_major": mentor_major, "mentor_number": mentor_number, "mentor_type": mentor_type})
...
def story_like():
   ...
   try:
       ...
       if action_receive == 'like':
           ...
           print(who_array)
           db.like.update_one({'number': int(number_receive), 'category': 'story', 'time': time_receive}, {'$set': {'who': who_array}})
...
def story_bookmark():
   ...
   try:
       ...
       story_bookmark = db.bookmark.find_one({'number': int(number_receive), 'category': 'story', 'time': time_receive})
       ...
       if action_receive == 'mark':
           ...
           who_array.append(my_number)
           print(who_array)
           db.bookmark.update_one({'number': int(number_receive), 'category': 'story', 'time': time_receive}, {'$set': {'who': who_array}})
...
def resume_like():
   ...
   try:
       ...
       if action_receive == 'like':
           ...
           who_array.append(my_number)
           print(who_array)
           db.like.update_one({'number': int(number_receive), 'category': 'resume', 'time': time_receive}, {'$set': {'who': who_array}})
...
def resume_bookmark():
   ...
   try:
       ...
       resume_bookmark = db.bookmark.find_one({'number': int(number_receive), 'category': 'resume', 'time': time_receive})
       ...
       if action_receive == 'mark':
           ...
           who_array.append(my_number)
           print(who_array)
           db.bookmark.update_one({'number': int(number_receive), 'category': 'resume', 'time': time_receive}, {'$set': {'who': who_array}})
...
def record_bookmark():
   ...
   try:
       ...
       if action_receive == 'mark':
           ...
           who_array.append(my_number)
           print(who_array)
           db.bookmark.update_one({'number': int(number_receive), 'category': 'recordpaper'}, {'$set': {'who': who_array}})
...
def reply_story():
   ...
   try:
       ...
       db.reply.update_one({'number': mentor_num, 'category': 'story', 'time': which_story}, {'$set': {'reply': reply_array}})
       if (status != 'mentor') or (int(payload['number']) != mentor_num):
           print('alert insert')
           doc = {
               'to_status': 'mentor',
...
def reply_resume():
   ...
   try:
       ...
       db.reply.update_one({'number': mentor_num, 'category': 'resume', 'time': which_resume}, {'$set': {'reply': reply_array}})
       if (status != 'mentor') or (int(payload['number']) != mentor_num):
           print('alert insert')
           doc = {
               'to_status': 'mentor',
...
def reply_record():
   ...
   try:
       ...
       db.reply.update_one({'number': mentor_num, 'category': 'recordpaper', 'time': which_record}, {'$set': {'reply': reply_array}})
       if (status != 'mentor') or (int(payload['number']) != mentor_num):
           print('alert insert')
           doc = {
               'to_status': 'mentor',
...
def charge_readypass():
   ...
   return render_template('charge.html',product=product, me_info=me_info,action_mentor=action_mentor_array,nonaction_mentor=nonaction_mentor_array,status=status,my_alert=my_alert,token_receive=token_receive)
...
def search():
   ...
   return render_template('search.html',selectedUnivArray=selectedUnivArray,selectedMajorArray=selectedMajorArray,selectedTypeArray=selectedTypeArray,tag=tag,ft=ft,st=st, me_info=me_info, action_mentor=action_mentor_array,
...
def get_product():
   ...
   for number_str in Arr:
       ...
       if db.recordpaper.find_one({'number':number})['record_title'] != '':
           db_record = db.recordpaper.find_one({'number':number})
       ...
       if db.resume.find({'number':number}) is not None:
           db_resume = list(db.resume.find({'number': number}))
           for resume in db_resume:
               if db.like.find_one({'number': number, 'category': 'resume'}) is not None:
                   like = len(db.like.find_one({'number': number, 'category': 'resume'})['who'])
               else:
                   like = 0

               if db.reply.find_one({'number': number, 'category': 'resume'}) is not None:
                   reply = str(db.reply.find_one({'number': number, 'category': 'resume'})['reply']).count('일')
               else:
                   reply = 0
               res_doc = {
                   'img': img,
       ...
       if db.story.find({'number': number}) is not None:
           db_story = list(db.story.find({'number': number}))
           for story in db_story:
               if db.like.find_one({'number': number, 'category': 'story'}) is not None:
                   like = len(db.like.find_one({'number': number, 'category': 'story'})['who'])
               else:
                   like = 0

               if db.reply.find_one({'number': number, 'category': 'story'}) is not None:
                   reply = str(db.reply.find_one({'number': number, 'category': 'story'})['reply']).count('일')
               else:
                   reply = 0
               sto_doc = {
                   'img': img,
...
def mentor_product(mentor_number):
   ...
   return render_template('mentor_products.html',ft=ft_receive,mentor_info=mentor_info,mentorinfo_info=mentorinfo_info,myFeed=myFeed, me_info=me_info,action_mentor=action_mentor_array,nonaction_mentor=nonaction_mentor_array, status=status,follower=mentor_follower,followed=followed, my_alert=my_alert,token_receive=token_receive)
...
def recordpaper_sell(mentor_number):
    ...
    try:
        ...
        if db.pay.find_one({'category':'recordpaper', 'client_num':me_info['number'], 'number':mentor_number}) is not None:
            buythis = db.pay.find_one({'category':'recordpaper', 'client_num':me_info['number'], 'number':mentor_number})['exp_time']
        else:
            buythis = ""

        return render_template('recordpaper_sell.html', mentor_info=mentor_info,record=record,this_like=this_like,this_reply=this_reply,resume_array=resume_array,story_array=story_array,mentorinfo_info=mentorinfo_info, myFeed=myFeed, me_info=me_info, buythis=buythis,action_mentor=action_mentor_array, nonaction_mentor=nonaction_mentor_array, status=status,follower=mentor_follower, followed=followed, my_alert=my_alert, token_receive=token_receive)
    except:
        return render_template('recordpaper_sell.html', mentor_info=mentor_info, record=record,this_like=this_like,this_reply=this_reply,resume_array=resume_array,story_array=story_array,mentorinfo_info=mentorinfo_info, myFeed=False, me_info=None, buythis=None,action_mentor=action_mentor_array, nonaction_mentor=nonaction_mentor_array,status=None,token_receive=token_receive)
...
def resume_sell(mentor_number, time):
    ...
    #resume_info
    resume = db.recordpaper.find_one({'number': mentor_number, 'time':time})
    this_like = len(db.like.find_one({'number':mentor_number,'category':'resume', 'time':time})['who'])
    this_reply = str(db.reply.find_one({'number':mentor_number,'category':'resume', 'time':time})['reply']).count('일')
    ...
    record_array = []
    if recordpaper is not None:
        if db.like.find_one({'number': mentor_number, 'category': 'racordpaper'}):
            record_like = len(
    ...
    try:
        ...
        return render_template('resume_sell.html', mentor_info=mentor_info,resume=resume,time=time,this_like=this_like,this_reply=this_reply,record_array=record_array,resume_array=resume_array,story_array=story_array,mentorinfo_info=mentorinfo_info, myFeed=myFeed, me_info=me_info, buythis=buythis,action_mentor=action_mentor_array, nonaction_mentor=nonaction_mentor_array, status=status,follower=mentor_follower, followed=followed, my_alert=my_alert, token_receive=token_receive)
    except:
        return render_template('resume_sell.html', mentor_info=mentor_info, resume=resume,time=time,this_like=this_like,this_reply=this_reply,record_array=record_array,resume_array=resume_array,story_array=story_array,mentorinfo_info=mentorinfo_info, myFeed=False, me_info=None, buythis=None,action_mentor=action_mentor_array, nonaction_mentor=nonaction_mentor_array,status=None,token_receive=token_receive)
